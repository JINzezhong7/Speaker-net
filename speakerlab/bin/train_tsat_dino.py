...
def main():
    ...
    tsat_model = build('TSAT_model',config)

    # synchronize batch norms
    student = nn.SyncBatchNorm.convert_sync_batchnorm(student)
    teacher = nn.SyncBatchNorm.convert_sync_batchnorm(teacher)
    tsat_model = nn.SyncBatchNorm.convert_sync_batchnorm(tsat_model)
    student.cuda()
    teacher.cuda()
    tsat_model.cuda()
    ...
    tsat_model = nn.parallel.DistributedDataParallel(tsat_model)
    ...
    # Prepare student net optimizer
    params_groups = utils_rdino.get_params_groups(student)
    optimizer = torch.optim.SGD(params_groups, lr=0, momentum=0.9)

    # Prepare tsat net optimizer
    params_groups = utils_rdino.get_params_groups(tsat_model)
...
def train_one_epoch(student, teacher, dino_loss, train_loader, optimizer, lr_schedule, \
    ...
    for it, data in enumerate(
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            teacher_output = teacher(global_data)
            student_global_output = student(global_data)
            ...
            student_local_output = student(local_data)
